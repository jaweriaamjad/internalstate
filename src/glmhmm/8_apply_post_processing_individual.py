...
from glm_hmm_utils import load_subject_list
...
for subject in subject_list:
    ...
    for fold in range(num_folds):
        ...
        for model in models:
            ...
            if model == "GLM":
                ...
                glm_weights_file = join(overall_dir, 'GLM', 'fold_' + str(fold),
    ...
    # Save best initialization directories across subjects,
    # folds and models (only GLM-HMM):
    print(cvbt_folds_model)
    print(cvbt_train_folds_model)
    json_dump = json.dumps(best_init_cvbt_dict)
    f = open(join(overall_dir, "best_init_cvbt_dict.json"), "w")
    f.write(json_dump)
    f.close()
    # Save cvbt_folds_model as numpy array for easy parsing
    # across all models and folds
    np.savez(join(overall_dir, "cvbt_folds_model.npz"), cvbt_folds_model)
    np.savez(join(overall_dir, "cvbt_train_folds_model.npz"), cvbt_train_folds_model)