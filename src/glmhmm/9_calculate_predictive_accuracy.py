...
subject_params_dict = dict()
for subject in subject_list:
    print(subject)

    subject_params_dict[subject] = dict()

    results_dir = join(overall_dir, subject)
    ...
    for K in range(2, MAX_K+1):
        with open(join(results_dir, "best_init_cvbt_dict.json"), 'r') as f:
            ...
            subject_params_dict[subject][K] = dict(
                weight_vectors = dict(
            ...
            subject_params_dict[subject][K]['predictive_acc'] = \
                    predictive_acc_this_K
...
with open(join(dict_dir, 'subject_results_dict.json'), 'w') as f:
    f.write(json.dumps(subject_params_dict))