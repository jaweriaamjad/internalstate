...
def get_raw_data(eid, one):
    ...
    return (subject, trials.contrastLeft, trials.contrastRight, trials.feedbackType, trials.choice,
...
def create_previous_choice_vector(choice):
    ''' choice: choice vector of size T
        ...
        subject - output is in {0, 1}, where 0 corresponds to a previous left
...
def get_all_unnormalized_data_this_session(eid, one):
    ...
    # Keep all trials
    trials_to_study = np.arange(len(bias_probs)) # pointless.. just to keep the code as it was below
    num_viols = len(np.where(choice[trials_to_study] == 0)[0])
    to_save = False
    if num_viols < .1 * len(trials_to_study):
        to_save = True
        ...
        bias_probs = np.expand_dims(bias_probs[trials_to_study], axis=1)

    return subject, unnormalized_inpt, y, session, num_viols, rewarded, bias_probs, to_save
...
def create_train_test_sessions(session, num_folds=3):
    ...
    # AP: this check should be changed when the number of sessions is small
    # or comparable with the number of folds
    # It did not create a problem with > 30 session (ZA pipeline), because
    # it was almost impossible to leave out one of the fold values out of
    # the 30+ randomly selected shuffled fold values.
    if len(np.unique(shuffled_folds)) != num_folds:
            print("require at least one session per fold for each subject!")
            shuffled_folds = np.random.choice(np.arange(num_folds), size=num_sessions)
...
def load_subject_list(list_file):
...
def fit_glm_hmm(datas, inputs, masks, K, D, M, C, N_em_iters,
                ...
                params_for_initialization, save_title, noise_sigma=0.2):
    '''
    ...
    if global_fit == True:
        ...
        glm_vectors_with_noise = glm_vectors_repeated + np.random.normal(
            0, noise_sigma, glm_vectors_repeated.shape)
    ...
    print('hmm params', this_hmm.params)
    print('lls', lls)
