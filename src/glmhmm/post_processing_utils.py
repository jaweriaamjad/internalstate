...
def calculate_glm_hmm_test_loglikelihood(glm_hmm_dir, test_datas, test_inputs,
    ...
    :return:
    """
    this_file_name = join(glm_hmm_dir, 'iter_*', 'glm_hmm_raw_parameters_*.npz')
...
def return_glmhmm_nll(inpt, y, session, session_fold_lookup_table, fold, K, D,
    ...
    train_inputs, train_datas, train_nonviolation_masks = \
        partition_data_by_session(
            ...
            this_train_session)
    dir_to_check = join(results_dir_glm_hmm, 'GLM_HMM_K_' + str(K), 'fold_' + str(fold))
...
def get_file_name_for_best_model_fold(cvbt_folds_model, K, overall_dir,
    ...
    key_for_dict = join('GLM_HMM_K_' + str(K), 'fold_' + str(best_fold))
...
def calculate_state_permutation(hmm_params):
    ...
    states_rem = list(range(K))
    if K == 3:
        # pick the engaged state as the one with largest weight for stimulus
        engaged_loc = np.argmax(glm_weights[:, 0, 0])
        states_rem.remove(engaged_loc)
        # pick the remaining states in order of increasing bias
        biases_rem = np.take(glm_weights[:,0,-1], states_rem)
        bias_left_loc, bias_right_loc = np.array(states_rem)[np.argsort(biases_rem)]

        permutation = np.array([engaged_loc, bias_left_loc, bias_right_loc])
    elif K == 4:
        # When running on biased blocks, we expect to see 2 engaged states
        # with opposite biases and 2 "disengaged" states with opposite biases

         # find the engaged states as the two with largest weight for stimulus
        engaged_locs = np.argsort(glm_weights[:,0,0])[-2:]

        # pick them in order of increasing bias
        engaged_biases = np.take(glm_weights[:,0,-1], engaged_locs)
        engaged_left_loc, engaged_right_loc = np.array(engaged_locs)[np.argsort(engaged_biases)]
        # remove them from remaining states
        states_rem.remove(engaged_left_loc)
        states_rem.remove(engaged_right_loc)
        ...
        permutation = np.array([
                engaged_left_loc, bias_left_loc,
                engaged_right_loc, bias_right_loc,
            ])

    else:
        # order states by engagement: with the most engaged being first
        # argsort sorts in ascending order, so indices are picked in reversed order
        permutation = np.argsort(glm_weights[:, 0, 0])[::-1]

    # assert that all indices are present in permutation exactly once:
    assert len(permutation) == K, "permutation is incorrect size"
    assert check_all_indices_present(permutation, K), \
        "not all indices present in permutation: " + str(permutation)
    return permutation
