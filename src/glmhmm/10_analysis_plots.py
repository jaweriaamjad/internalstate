...
figure_dir = join(results_path, 'glmhmmlogs', tag, 'figures',  'individual', f'{K}_states')
...
for i, subject in enumerate(subjects):
    ...
    try:
        left_prob = load_bias(join(data_dir, subject + '_bias_probs.npz'))
        ...
        for n, eid in enumerate(eids):
            ...
            print(f"\t{n+1}/{len(eids)}", eid, \
                "\t", len(trials.probabilityLeft) - 90, \
                "\t", len(np.where(session == eid)[0]))

        np.savez(join(data_dir, subject + '_bias_probs.npz'), left_prob)
        all_trials.to_df().to_parquet( join(data_dir, subject + '_trials_table.pqt') )
    ...
    #
    # sess_to_plot = sess_df.loc[sess_df.subject == subject].index.to_list()[-3:]
    plot_example_sessions(sess_to_plot, left_prob, posterior_probs, 
            nonviolation_idx, inpt, y, session, figure_dir, subject,
    ...
    plot_GLMHMM_results(-hmm_params[2], np.exp(hmm_params[1][0]), pred_acc_arr_for_plotting, \
        None, None, figure_dir, subject=subject, fig_name=f"{subject}.png")
